...
def _count(cur,sql,params=()):
    cur.execute(sql,params);row=cur.fetchone();return int(row[0] or 0) if row else 0
...
def build_reconciliation(cur,league_id,draft_id,draft_status):
    ...
    identity_errors=[]
    if not session:identity_errors.append("No authoritative draft session")
    else:
        if str(session[0])!=str(draft_id):identity_errors.append("Authoritative draft ID mismatch")
        if str(session[1])!=str(league_id):identity_errors.append("Authoritative league ID mismatch")
    ...
    sync=cur.fetchone();sync_errors=[]
    if not sync:sync_errors.append("No draft sync audit")
    elif str(sync[0]).upper()!="SUCCESS":sync_errors.append("Latest draft sync did not succeed")
    ...
    reconcile_errors=[]
    if not pre:
        if sleeper_count!=board_count:reconcile_errors.append("Sleeper pick count differs from drafted board count")
        if sleeper_count!=roster_count:reconcile_errors.append("Sleeper pick count differs from league roster count")
    ...
    invariant_errors=[]
    if multiple:invariant_errors.append("Players have multiple owners")
    if roster_not_drafted:invariant_errors.append("Roster players are not marked drafted")
    if drafted_without_owner:invariant_errors.append("Drafted players have no owner")
    if my_orphans:invariant_errors.append("My roster contains orphan players")
    sections={
      "identity":{"status":"PASS" if not identity_errors else "FAIL","errors":identity_errors,"session":session},
      "sync":{"status":"PASS" if not sync_errors else "FAIL","errors":sync_errors,"latest":sync},
      "reconciliation":{"status":"PASS" if not reconcile_errors else "FAIL","errors":reconcile_errors,"counts":{"sleeper_picks":sleeper_count,"draft_board":board_count,"league_rosters":roster_count},"pre_draft":pre},
      "quarantine":{"status":"PASS" if quarantine_count==0 else "ATTENTION","open_count":quarantine_count},
      "invariants":{"status":"PASS" if not invariant_errors else "FAIL","errors":invariant_errors,"counts":{"multiple_owners":multiple,"roster_not_drafted":roster_not_drafted,"drafted_without_owner":drafted_without_owner,"my_roster_orphans":my_orphans}}
    }
...
def record_reconciliation(cur,league_id,draft_id,draft_status,result):
    import json
    c=result["sections"]["reconciliation"]["counts"];q=result["sections"]["quarantine"]["open_count"]
    cur.execute("""INSERT INTO draft_reconciliation_runs(draft_id,league_id,draft_status,overall_status,
      ...
      (str(draft_id),str(league_id),str(draft_status or ""),result["overall"],c["sleeper_picks"],c["draft_board"],c["league_rosters"],q,json.dumps(result,default=str)))

def build_draft_readiness(cur,league_id,season,signals=None,model_health=None):
    signals=signals or {};draft_status=str(signals.get("draft_status") or "").lower();thresholds=PREDRAFT_THRESHOLDS if draft_status in ("pre_draft","predraft") else LIVE_THRESHOLDS;health=model_health or {};snapshots={};deductions=[]
    ...
    score=max(0,100-sum(p for _,p in deductions));dp=snapshots.get("draft_picks",{});mode="LIVE" if dp.get("state")=="LIVE" else ("CACHED" if dp.get("state") in ("AGING","STALE") else "UNAVAILABLE");status="READY" if score>=85 else ("CAUTION" if score>65 else "NOT READY")
    ...
    return {"score":score,"status":status,"mode":mode,"checks":checks,"snapshots":snapshots,"deductions":[{"check":k,"points":p} for k,p in deductions],"calibration_active":bool(health.get("active")),"resolved_predictions":int(health.get("resolved") or 0),"draft_id":signals.get("draft_id"),"draft_status":signals.get("draft_status"),"user_slot":signals.get("user_slot"),"pick_count":int(signals.get("pick_count") or 0),"next_pick":signals.get("next_pick"),"draft_day":reconciliation,"overall":reconciliation["overall"]}
