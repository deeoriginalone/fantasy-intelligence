...
from pickem_pg_context import build_pickem_context as build_pg_pickem_context
...
def pickem_page():
    ...
    strategy = request.args.get("strategy", "balanced")
    return render_template("pickem.html", **build_pg_pickem_context(season, week, strategy))
...
def pickem_api():
    ...
    week = request.args.get("week", type=int) or current_app.config.get("NFL_WEEK", 1)
    strategy = request.args.get("strategy", "balanced")
    return jsonify(build_pg_pickem_context(season, week, strategy))
